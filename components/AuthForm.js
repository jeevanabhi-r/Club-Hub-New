function AuthForm() {
  ...
  React.useEffect(() => {
    ...
    const initAdmin = async () => {
      try {
        const res = await trickleListObjects('user', 100, false);
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const res = await trickleListObjects('user', 1000, false);
      const userObj = res.items.find(u => u.objectData.email === email && u.objectData.password === password);
      
      if (userObj) {
        const userData = { id: userObj.objectId, ...userObj.objectData, createdAt: userObj.createdAt };
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      setMsg({ type: 'error', text: 'Failed to register account.' });
    }
    setLoading(false);
  };
  ...
  const handleSendOtp = async (e) => {
    ...
    try {
      const res = await trickleListObjects('user', 1000, false);
      const userObj = res.items.find(u => u.objectData.email === email);
      
      if (userObj) {
        const generatedOtp = Math.floor(100000 + Math.random() * 900000).toString();
        const expiry = new Date(Date.now() + 5 * 60000).toISOString();
        
        await trickleUpdateObject('user', userObj.objectId, {
          ...userObj.objectData,
          otp: generatedOtp,
          otpExpiry: expiry
        });
  ...
  const handleVerifyOtp = async (e) => {
    ...
    try {
      ...
      if (userObj) {
        const isExpired = new Date(userObj.objectData.otpExpiry) < new Date();
        if (userObj.objectData.otp === otp && !isExpired) {
          await trickleUpdateObject('user', userObj.objectId, {
            ...userObj.objectData,
            password: password,
            otp: '',
