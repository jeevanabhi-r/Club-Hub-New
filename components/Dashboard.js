function Dashboard() {
  ...
  const fetchEvents = async () => {
    setLoading(true);
    try {
      const res = await trickleListObjects('event', 1000, false);
      const evts = res.items.map(item => ({
        id: item.objectId,
        ...item.objectData,
  ...
  const handleSaveEvent = async (e) => {
    ...
    try {
      ...
      if (formData.id) {
        await trickleUpdateObject('event', formData.id, payload);
      } else {
        await trickleCreateObject('event', payload);
      }
  ...
  const handleUpdateProfile = async (e) => {
    ...
    try {
      ...
      const res = await trickleGetObject('user', user.id);
      if (res) {
        ...
        const updatedUser = { ...user, name: newName, avatar: avatarPreview };
        setUser(updatedUser);
